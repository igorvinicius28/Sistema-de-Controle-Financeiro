...
namespace PrototipoLogin.Identity
{
    ...
    {
        //Manutencao (Avaliar por que os stamp estao diferentes)
        ...
        private static async Task<bool> VerifySecurityStampAsync(ApplicationUserManager manager, ApplicationUser user, CookieValidateIdentityContext context)
        {
            ...
            var stampteste = await manager.GetSecurityStampAsync(context.Identity.GetUserId());
            var flag = stamp == stampteste;
            //return flag;
            // return (stamp == await manager.GetSecurityStampAsync(context.Identity.GetUserId()));
            return stamp != null && stampteste != null;
        }
